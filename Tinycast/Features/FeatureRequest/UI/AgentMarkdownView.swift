...
private struct AgentMarkdownBlockView: View {
    ...
    var body: some View {
        ...
        case .heading(let level, let text):
            Text(AgentMarkdownInline.attributed(text, metrics))
                .font(AgentMarkdownInline.headingFont(level, metrics))
                .fixedSize(horizontal: false, vertical: true)
        case .paragraph(let text):
            Text(AgentMarkdownInline.attributed(text, metrics))
                .fixedSize(horizontal: false, vertical: true)
        case .bulletList(let items):
            AgentMarkdownListView(items: items, start: nil)
        case .numberedList(let start, let items):
            AgentMarkdownListView(items: items, start: start)
        case .code(let language, let text):
            AgentMarkdownCodeView(language: language, text: text)
        case .quote(let blocks):
            AgentMarkdownQuoteView(blocks: blocks)
...
private struct AgentMarkdownListView: View {
    ...
    @Environment(\.metrics) private var metrics
    let items: [AgentMarkdownBlock.Item]
        ...
        VStack(alignment: .leading, spacing: metrics.spacing.xs) {
            ForEach(Array(items.enumerated()), id: \.offset) { offset, item in
                HStack(alignment: .firstTextBaseline, spacing: metrics.spacing.sm) {
                    ...
                    AgentMarkdownView(blocks: item.blocks, spacing: metrics.spacing.xs)
                }
            }
        }
    }
...
private struct AgentMarkdownQuoteView: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: metrics.spacing.lg) {
            ...
            AgentMarkdownView(blocks: blocks)
                .foregroundStyle(Theme.Colors.textSecondary)
        }
        .fixedSize(horizontal: false, vertical: true)
    }
