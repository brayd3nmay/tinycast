...
/// Deliberately duplicated from `Features/AI/UI`: the agent's transcript and the AI chat's
/// must be free to diverge, and neither feature may own the other's look. See
/// docs/features/feature-request.md#why-the-transcript-is-duplicated.
...
enum AgentMarkdownBlock: Equatable, Sendable {
    ...
    static func parse(_ markdown: String) -> [AgentMarkdownBlock] {
        ...
        var reader = AgentMarkdownReader(lines: text.components(separatedBy: "\n"))
        return reader.blocks()
    }
}

private struct AgentMarkdownReader {
    let lines: [String]
    var index = 0

    mutating func blocks() -> [AgentMarkdownBlock] {
        var blocks: [AgentMarkdownBlock] = []
        while let line = peek() {
            if line.isBlankLine {
                index += 1
            } else if let fence = AgentMarkdownLine.fence(line) {
                blocks.append(code(fence))
            } else if AgentMarkdownLine.isRule(line) {
                index += 1
                blocks.append(.rule)
            } else if let heading = AgentMarkdownLine.heading(line) {
                index += 1
                blocks.append(heading)
            } else if AgentMarkdownLine.isQuote(line) {
                blocks.append(quote())
            } else if let table = table() {
                blocks.append(table)
            } else if let marker = AgentMarkdownLine.listMarker(line) {
    ...
    private mutating func code(_ fence: AgentMarkdownLine.Fence) -> AgentMarkdownBlock {
        index += 1
        var body: [String] = []
        while let line = peek(), !AgentMarkdownLine.closes(line, fence) {
            body.append(AgentMarkdownLine.dedent(line, by: fence.indent))
            index += 1
        }
        if peek() != nil { index += 1 }
    ...
    private mutating func quote() -> AgentMarkdownBlock {
        var inner: [String] = []
        while let line = peek() {
            if AgentMarkdownLine.isQuote(line) {
                inner.append(AgentMarkdownLine.strippingQuoteMarker(line))
            } else if line.isBlankLine || AgentMarkdownLine.startsBlock(line, interrupting: false) {
                break
            } else {
                inner.append(line)
            }
        ...
        var reader = AgentMarkdownReader(lines: inner)
        return .quote(reader.blocks())
    }

    private mutating func table() -> AgentMarkdownBlock? {
        guard let header = peek(), header.contains("|"), let delimiter = peek(1),
            let alignments = AgentMarkdownLine.tableAlignments(delimiter)
        else { return nil }
        let titles = AgentMarkdownLine.tableCells(header)
        guard titles.count == alignments.count else { return nil }
        index += 2
        var rows: [[String]] = []
        while let line = peek(), !line.isBlankLine, line.contains("|") {
            rows.append(AgentMarkdownLine.tableCells(line).resized(to: titles.count))
            index += 1
        }
        return .table(.init(header: titles, alignments: alignments, rows: rows))
    }

    private mutating func list(from first: AgentMarkdownLine.Marker) -> AgentMarkdownBlock {
        var items: [AgentMarkdownBlock.Item] = []
        while let line = peek(), let marker = AgentMarkdownLine.listMarker(line),
    ...
    private mutating func item(
        startingWith line: String, marker: AgentMarkdownLine.Marker
    )
        -> AgentMarkdownBlock.Item
    {
        ...
        while let line = peek() {
            if line.isBlankLine {
                ...
                AgentMarkdownLine.startsBlock(line, interrupting: false)
            {
                break
            }
            body.append(AgentMarkdownLine.dedent(line, by: marker.contentIndent))
            index += 1
        }
        var reader = AgentMarkdownReader(lines: body)
        var blocks = reader.blocks()
        guard case .paragraph(let text) = blocks.first, let task = AgentMarkdownLine.taskBox(text) else {
    ...
    private mutating func paragraph() -> AgentMarkdownBlock {
        var body: [String] = []
        while let line = peek(), !line.isBlankLine {
            if !body.isEmpty, AgentMarkdownLine.startsBlock(line, interrupting: true) || startsTable() {
                break
            }
...
private enum AgentMarkdownLine {
    ...
    static func heading(_ line: String) -> AgentMarkdownBlock? {
        let body = line.dropFirst(line.indent)
    ...
    static func tableAlignments(_ line: String) -> [AgentMarkdownBlock.Table.Alignment]? {
        guard line.contains("|") else { return nil }
        var alignments: [AgentMarkdownBlock.Table.Alignment] = []
